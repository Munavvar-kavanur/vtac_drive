...
export class GoogleDriveAdapter extends StorageAdapter {
    ...
    async getResumableUploadUrl(fileName, mimeType, fileSize, parentFolderId, origin) {
        try {
            // Note: We're using the drive folder ID from env, ignoring parentFolderId for now 
            // as the current Adapter implementation uses a single root folder (GOOGLE_DRIVE_FOLDER_ID)
            ...
            // Spoof the Client's Origin to satisfy Google's CORS consistency check
            const headers = {
            ...
            if (!location) {
                console.error('Drive Headers:', [...response.headers.entries()]);
